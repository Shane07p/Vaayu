...
function CitizenChrome({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#080b0d] text-slate-100 font-sans antialiased">
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#080b0d]/80 backdrop-blur-xl"><div className="mx-auto flex max-w-3xl items-center justify-between gap-2 px-4 py-3.5 sm:px-5">
      ...
      <nav className="flex items-center gap-1 sm:gap-2"><LanguageSelector /><Link href="/aqi" className="rounded-xl px-2 py-1.5 text-xs font-mono text-slate-200 sm:px-3">{t.myAir}</Link><Link href="/report" className="rounded-xl px-2 py-1.5 text-xs font-mono text-slate-400 sm:px-3">{t.report}</Link><Link href="/map" className="hidden rounded-xl px-3 py-1.5 text-xs font-mono text-teal-300 sm:block">{t.console} →</Link></nav>
    </div></header>
